 ...
 * @author wseyler
 */
public class PluginIT extends TestCase {
...
//                                               {47, 33, 51, "Europe",         "Germany",  "West"}}; //$NON-NLS-1$ //$NON-NLS-2$//$NON-NLS-3$
  ...
  private static final String TEST_FILE_PATH = "/target/test-output/TestChart/"; //$NON-NLS-1$
  ...
  private void testRenderAsPng(final String fileName,
    ...
    String chartFileName = System.getProperty("user.dir") + TEST_FILE_PATH + fileName.substring(0, fileName.indexOf('.')) + PNG_SUFFIX;
  ...
  private void testRenderAsJpeg(final String fileName,
    ...
    String chartFileName = System.getProperty("user.dir") + TEST_FILE_PATH + fileName.substring(0, fileName.indexOf('.')) + JPG_SUFFIX;
...
//      "PluginTest2f.xml", //$NON-NLS-1$
  ...
  private void runTests(final String[] fileNames, final ChartTableModel data) {
    ...
    File testFilePath = new File(System.getProperty("user.dir") + TEST_FILE_PATH);
...
//      }
  ...
  private void testRenderAsXml(final String fileName,
    ...
    String chartFileName = System.getProperty("user.dir") + TEST_FILE_PATH + fileName.substring(0, fileName.indexOf('.')) + ".xml";
  ...
  public void testOpenFlashBarChart() {
    ...
    try {
      inputStream = ChartBeanFactory.createChart(chartData, 1, false, 2, 0, 1, chartModel, null, 100, 100, null);
  ...
  public void testOpenFlashLineChart() {
    ...
    try {
      InputStream inputStream = ChartBeanFactory.createChart(chartData, 1, false, 2, 0, 1, chartModel, null, 100, 100, null);
      String jsonString = convertStreamToString(inputStream);
      for (int row = 0; row < chartData.length; row++) {
        for (int column = 0; column < chartData[row].length; column++) {
          if (column == 2) {
  ...
  public void testOpenFlashPieChart() {
    ...
    chartModel.setTheme(ChartTheme.THEME2);
    try {
      InputStream inputStream = ChartBeanFactory.createChart(chartData, 1, false, 1, 0, -1, chartModel, null, 100, 100, null);
      String jsonString = convertStreamToString(inputStream);
      for (int row = 0; row < chartData.length; row++) {
        for (int column = 0; column < chartData[row].length; column++) {
          if (column == 1) {
