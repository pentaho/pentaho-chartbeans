...
public class ChartMarkerHeightTest extends TestCase {
  ...
  public void testMarkerHeightStyle() throws Exception {
    final ChartDocumentContext cdc = ChartFactory.generateChart(getClass().getResource("ChartMarkerHeightTest.xml")); //$NON-NLS-1$
    ...
    while(child != null) {
      ...
      System.out.println("Expected: "+passValues[counter]+" - Got: "+layoutStyle.getValue(ChartStyleKeys.MARKER_HEIGHT)); //$NON-NLS-1$ //$NON-NLS-2$
      assertEquals(passValues[counter++].getCSSText(), layoutStyle.getValue(ChartStyleKeys.MARKER_HEIGHT).getCSSText());
